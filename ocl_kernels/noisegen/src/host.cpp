...
int main(int argc, char **argv) {
    ...
    OCL_CHECK(err,
              cl::Buffer buffer_victim(context,
    ...
    OCL_CHECK(err, cl::Kernel noisegen_kernel(program, "noisegen", &err));
    OCL_CHECK(err, cl::Kernel kernel_bench(program, "comm_kernel", &err));
    OCL_CHECK(err, err = noisegen_kernel.setArg(0, buffer_victim));
    OCL_CHECK(err, err = noisegen_kernel.setArg(1, idle));
    OCL_CHECK(err, err = noisegen_kernel.setArg(2, busy));
  
    OCL_CHECK(err, err = q.enqueueNDRangeKernel(noisegen_kernel, NULL, gSize, wgSize, NULL, NULL));
...
// 释放内核对象
if (noisegen_kernel() != NULL) {
    clReleaseKernel(noisegen_kernel.get());
}
